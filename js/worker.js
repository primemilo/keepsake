  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/clone") {
      try {
        ...
        if (!sample) {
          return new Response(JSON.stringify({ error: "no audio sample" }), { status: 400, headers: { ...CORS, "Content-Type": "application/json" } });
        }
        ...
        if (!resp.ok) {
          ...
          return new Response(JSON.stringify({ error: "fish clone failed", status: resp.status, detail }), { status: 502, headers: { ...CORS, "Content-Type": "application/json" } });
        }
        const model = await resp.json();
        return new Response(JSON.stringify({ id: model._id }), { headers: { ...CORS, "Content-Type": "application/json" } });
      } catch (e) {
    ...
    try {
      ...
      if (!resp.ok) { return new Response("fish tts failed: " + resp.status, { status: 502, headers: CORS }); }
      return new Response(resp.body, { headers: { ...CORS, "Content-Type": "audio/mpeg" } });
    } catch (e) {
